...
func sendMessageFannout(connection *amqp.Connection) {
	...
	err = channel.ExchangeDeclare("fanout-event", "fanout", true, false, false, false, nil)
	...
	for loop {
		...
		case <-time.After(time.Second):
			...
			err = channel.Publish("fanout-event", "", false, false, message)
...
func receiveMessageFannout(connection *amqp.Connection) {
	// Create a channel from the connection. We'll use channels to access the data in the queue rather than the
	// connection itself
	channel, err := connection.Channel()
	...
	q, err := channel.QueueDeclare("que-fanout", true, false, false, false, nil)
	...
	err = channel.QueueBind(q.Name, "", "fanout-event", false, nil)
